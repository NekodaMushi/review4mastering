...
export default function SignUpScreen() {
  ...
  async function handleSubmit() {
    ...
    try {
      const result = await authPost("/api/auth/sign-up/email", {
        name,
        email: trimmedEmail,
        password,
      });
  ...
  const inputClass =
    "w-full rounded-lg bg-neutral-900 border border-neutral-800 px-4 py-3 text-sm text-white";
        ...
        <View className="w-full max-w-md self-center space-y-6">
          ...
          <Text className="text-center text-sm text-neutral-500">
            ...
            <Link href="/(auth)/sign-in">
              <Text className="text-amber-400">Login</Text>
            </Link>
          </Text>
        </View>
      </ScrollView>
