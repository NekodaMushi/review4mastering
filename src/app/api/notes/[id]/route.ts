...
import { z } from "zod";
import { RevisionStage, ActionType } from "@prisma/client";
...
const STAGE_DURATIONS: Record<RevisionStage, number> = {
  TEN_MINUTES: 10,
  ONE_DAY: 24 * 60,
  SEVEN_DAYS: 7 * 24 * 60,
  ONE_MONTH: 30 * 24 * 60,
...
function calculateNextRevision(stage: RevisionStage): Date {
  if (stage === "COMPLETED") {
...
export async function PATCH(
  ...
  try {
    ...
    const nextRevision = calculateNextRevision(newStage);
    ...
    await prisma.revision_history.create({
      data: {
        note_id: id,
